...
namespace UUJ.GuidersHub.Tests
{
    public abstract class GuidersHubTestBase : AbpIntegratedTestBase<GuidersHubTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected GuidersHubTestBase()
        {
        ...
        protected void UsingDbContext(Action<GuidersHubDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GuidersHubDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GuidersHubDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GuidersHubDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GuidersHubDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GuidersHubDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GuidersHubDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GuidersHubDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GuidersHubDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GuidersHubDbContext, Task<T>> func)
        {
            T result;
